...
import { findUserByEmail, findUserById, createUser, generateToken, setAuthCookie, generateId } from '@/lib/auth';

// POST /api/auth/parent-token
// Validates a parent app's Supabase token and creates a CRM session
export async function POST(request: NextRequest) {
  try {
    ...
    // Find or create a CRM user with this email
    ...
    if (!existingUser) {
      ...
      try {
        const newUser = await createUser(
          validation.email,
          randomPassword,
          undefined, // fullName - they can set it later
          undefined  // companyName - they can set it later
        );
    ...
    // Generate a CRM JWT and set the auth cookie
